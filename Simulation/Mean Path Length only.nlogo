...
to setup
  ...
  clear-all
  file-close-all
  set-current-directory "//Users//wangyaofeng//Desktop//Ant_Research//Data_Files//3" ; set whatever directory you want to save the files here
  carefully [
  ...
  ;  file-delete (word "Nest pheromone record " population ".csv")]
     file-delete (word "Mean Path Length.csv")] []; delete all files
  reset-ticks
  set currentSim 1
  reset
end
...
to export-data ;;CSV is better extension then xls, even though it will be opened by excel
  ...
  file-open (word "Mean Path Length.csv") ;The new result append to the old file
  ifelse (currentsim = 1) [record-condition
  file-print (word "Tick Marks," "Mean Path Length")] [file-print ""]
  file-print csv:to-row (list "Current Sim: " currentsim)
  ;file-print (word "Current Sim: " currentsim)
