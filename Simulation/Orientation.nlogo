globals     [name nest-color time food-color nestPaths totalPaths nestTicks xrecord foodname nestname yrecord currentSim tickMarks heading-histogram current-heading OrientationTicks FoodPheromeonRecord NestPheromeonRecord] ;; setting global variables (description below)
...
to go
  ...
  ifelse (currentSim <= simNUM)[
   ...
   record-condition
   file-print (word "Name, " "Ticks, " "Bin, " "x-cor, " "y-cor")
   reset-ticks
  ...
  (foreach name OrientationTicks
    ;heading-histogram
    xrecord yrecord [file-print csv:to-row  (list ?1 ?2 ?3 ?4)])
  file-close
  ;export-data
...
to export-data ;;CSV is better extension then xls, even though it will be opened by excel
  ...
  ;if (currentsim = 1) [record-condition
  ;file-print (word "Tick Marks," "Path Length")]
  ;file-print (word "Current Simulation: " currentsim)
  ;(foreach TickMarks TotalPaths foodname [file-print csv:to-row (list ?1 ?2)])
  ;file-close
  ...
  ;file-print (word "Tick Marks," "Path Length")]
  ;file-print (word "Current Simulation: " currentsim)
  ;(foreach nestTicks nestPaths nestname[file-print csv:to-row (list ?1 ?2)])
  ;file-close
end
